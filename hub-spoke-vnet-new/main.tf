...
resource "azurerm_network_security_group" "claims" {
  name                = "dremio-nsg"
  location            = "${azurerm_resource_group.vnet_gp.location}"
  resource_group_name = "${azurerm_resource_group.vnet_gp.name}"
}

resource "azurerm_subnet" "claims" {
  name                      = "dremio"
  virtual_network_name      = "${azurerm_virtual_network.claims.name}"
  resource_group_name       = "${azurerm_resource_group.vnet_gp.name}"
  address_prefix            = "11.0.1.0/24"
  #network_security_group_id = "${azurerm_network_security_group.claims.id}"
}
resource "azurerm_subnet_network_security_group_association" "claims" {
  subnet_id                 = "${azurerm_subnet.claims.id}"
  network_security_group_id = "${azurerm_network_security_group.claims.id}"
}
...
resource "azurerm_virtual_network_peering" "peer1" {
  ...
  remote_virtual_network_id = "${azurerm_virtual_network.claims.id}"
